/**
 *	IDB Class with lS fallback
 *	This indexedDB abstraction Class gracefully falls back to localStorage.
 *
 *	IDB based on: https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API
**/

var StorageClassIDB = function(dbname, version, callback) {
	...
	this.get = function(type, selector, callback) {
		...
		// DB call
		var transaction = db.transaction([type]);		
		transaction.onerror = error;
				
		var objectStore = transaction.objectStore(type);
		var request = objectStore.get(selector);
	 ...
	 *
	 * Update existing or insert a new entry or list of entries in local storage
	 */
	 ...
	 this.set = function(type, content, callback) {
		...
		for(var i in content)
			var request = objectStore.put(content[i]);
	 ...
	 */
	 ...
	 this.remove = function(type, selector, content, callback) {
		...
		var request = db.transaction([type], "readwrite")
            .objectStore(type)
            .delete(selector);
		
		if(callback) request.onsuccess = callback;
